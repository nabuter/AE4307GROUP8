/* ------------------------------------------------------------------   */
/*      item            : WorldViewAdapter.cxx
        made by         : tux
	from template   : DusimeModuleTemplate.cxx
        template made by: Rene van Paassen
        date            : Wed May 13 23:32:39 2015
	category        : body file 
        description     : 
	changes         : Wed May 13 23:32:39 2015 first version
	template changes: 030401 RvP Added template creation comment
	                  060512 RvP Modified token checking code
        language        : C++
*/

#define WorldViewAdapter_cxx
// include the definition of the module class
#include "WorldViewAdapter.hxx"
...
// class/module name
const char* const WorldViewAdapter::classname = "world-view-adapter";

// initial condition/trim table
const IncoTable* WorldViewAdapter::getMyIncoTable()
{
...
//     new VarProbe<WorldViewAdapter,double>
//       (REF_MEMBER(&WorldViewAdapter::i_example))}
...
const ParameterTable* WorldViewAdapter::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    { "set-timing", 
      new MemberCall<WorldViewAdapter,TimeSpec>
        (&WorldViewAdapter::setTimeSpec), set_timing_description },

    { "check-timing", 
      new MemberCall<WorldViewAdapter,vector<int> >
      (&WorldViewAdapter::checkTiming), check_timing_description },
...
// constructor
WorldViewAdapter::WorldViewAdapter(Entity* e, const char* part, const
		       PrioritySpec& ps) :
  ...
  // myclock(),
  cb1(this, &WorldViewAdapter::doCalculation),
  do_calc(getId(), "This is an adapter that prepares the information to be presented in the WorldView module", &cb1, ps)
{
...
bool WorldViewAdapter::complete()
{
...
// destructor
WorldViewAdapter::~WorldViewAdapter()
{
  //
}

// as an example, the setTimeSpec function
bool WorldViewAdapter::setTimeSpec(const TimeSpec& ts)
{
...
bool WorldViewAdapter::checkTiming(const vector<int>& i)
{
  if (i.size() == 3) {
...
bool WorldViewAdapter::isPrepared()
{
  bool res = true;
...
// start the module
void WorldViewAdapter::startModule(const TimeSpec &time)
{
  do_calc.switchOn(time);
}

// stop the module
void WorldViewAdapter::stopModule(const TimeSpec &time)
{
...
void WorldViewAdapter::fillSnapshot(const TimeSpec& ts, 
...
// declaration) if you specified to the SimulationModule that the size of 
// state snapshots is zero
void WorldViewAdapter::loadSnapshot(const TimeSpec& t, const Snapshot& snap)
{
...
// appropriate output
void WorldViewAdapter::doCalculation(const TimeSpec& ts)
{
...
void WorldViewAdapter::trimCalculation(const TimeSpec& ts, const TrimMode& mode)
{
...
static TypeCreator<WorldViewAdapter> a(WorldViewAdapter::getMyParameterTable());
