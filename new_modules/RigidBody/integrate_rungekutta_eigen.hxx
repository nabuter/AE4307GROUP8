/* ------------------------------------------------------------------   */
/*      item            : integrate_rungekutta_eigen.hxx
...
#ifndef integrate_rungekutta_eigen_hxx
#define integrate_rungekutta_eigen_hxx
...
/** \file integrate_rungekutta_eigen.hxx
...
/** This defines a "data-pack", with room for workspace for the
    ...
    size. */
class EigenRungeKuttaWorkspace
{
  /** Data space */
  double *data;
...
public:
  /** STL vectors. \{ */
  VectorE k1, k2, k3, k4, xhold, xwork; /// \}

  /** Constructor.
      \param size    Size of the state vector for integration. */
  EigenRungeKuttaWorkspace(unsigned size);

  /** Constructor with external data.
      The data must hold 6xsize variables */
  EigenRungeKuttaWorkspace(double *data, unsigned int size);

  /** Destructor. */
  ~EigenRungeKuttaWorkspace();
};
...
template <class MOD>
void integrate_rungekutta_eigen(MOD &model, EigenRungeKuttaWorkspace &ws, double dt)
{
  // first step
  // k1 = T f(x, t) 
