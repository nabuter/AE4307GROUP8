...
/*      item            : DataLogger.cxx
	...
	from template   : DusimeModuleTemplate.cxx
        ...
        date            : Wed May 13 23:23:04 2015
	category        : body file 
        description     : 
	changes         : Wed May 13 23:23:04 2015 first version
	template changes: 030401 RvP Added template creation comment
	                  060512 RvP Modified token checking code
        language        : C++
*/

#define DataLogger_cxx
...
// class/module name
const char* const DataLogger::classname = "data-logger";

// initial condition/trim table
const IncoTable* DataLogger::getMyIncoTable()
{
...
//     new VarProbe<DataLogger,double>
//       (REF_MEMBER(&DataLogger::i_example))}
...
const ParameterTable* DataLogger::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    { "set-timing", 
      new MemberCall<DataLogger,TimeSpec>
        (&DataLogger::setTimeSpec), set_timing_description },

    { "check-timing", 
      new MemberCall<DataLogger,vector<int> >
      (&DataLogger::checkTiming), check_timing_description },
...
// constructor
DataLogger::DataLogger(Entity* e, const char* part, const
		       PrioritySpec& ps) :
  ...
  // myclock(),
  cb1(this, &DataLogger::doCalculation),
  do_calc(getId(), "This module logs all the input data from the stick and all the output data from the Dynamics Module", &cb1, ps)
{
...
bool DataLogger::setTimeSpec(const TimeSpec& ts)
{
...
bool DataLogger::checkTiming(const vector<int>& i)
{
  if (i.size() == 3) {
...
// start the module
void DataLogger::startModule(const TimeSpec &time)
{
  do_calc.switchOn(time);
}

// stop the module
void DataLogger::stopModule(const TimeSpec &time)
{
...
void DataLogger::fillSnapshot(const TimeSpec& ts, 
...
// declaration) if you specified to the SimulationModule that the size of 
// state snapshots is zero
void DataLogger::loadSnapshot(const TimeSpec& t, const Snapshot& snap)
{
...
// appropriate output
void DataLogger::doCalculation(const TimeSpec& ts)
{
...
void DataLogger::trimCalculation(const TimeSpec& ts, const TrimMode& mode)
{
...
static TypeCreator<DataLogger> a(DataLogger::getMyParameterTable());
