...
"$Id: Display.cxx,v 1.2 2015/05/06 16:46:34 stegginamanish Exp $";
...
Display::Display(Entity* e, const char* part, const
  ...
  mySignalStreamReadToken(getId(), NameSet(getEntity(), "SignalStream", part)),
  myDisplayColorEventReadToken(getId(), NameSet(getEntity(), "DisplayColorEvent", part)),
  ...
  do_calc(getId(), "draw display", &cb1, ps)
{
  ...
  do_calc.setTrigger(mySignalStreamReadToken);
...
bool Display::isPrepared()
{
  ...
  CHECK_TOKEN(mySignalStreamReadToken);
  CHECK_TOKEN(myDisplayColorEventReadToken);
...
void Display::doCalculation(const TimeSpec& ts)
{
  ...
  case SimulationState::HoldCurrent: {
    ...
    D_MOD("The simulation is displaying the current state\n");
    break;
    }
  ...
  case SimulationState::Advance: {
    ...
    D_MOD("The simulation is displaying the next state\n");
...
if (myDisplayColorEventReadToken.getNumWaitingEvents(ts))
{
try
{
EventReader<DisplayColorEvent> myDisplayColorEventReader(myDisplayColorEventReadToken, ts);
myDisplay.adjust(myDisplayColorEventReader.data().color);
}
catch (Exception& e)
{
W_MOD(classname << ": Error while reading Display Color Event!");
}
}

try
{
StreamReader<SignalStream> mySignalStreamReader(mySignalStreamReadToken, ts);
myDisplay.setAmplitude(mySignalStreamReader.data().signal);
myDisplay.redraw();
}
catch (Exception& e)
{
