...
//     new VarProbe<SignalGenerator,double>
//       (REF_MEMBER(&SignalGenerator::i_example))}
...
const ParameterTable* SignalGenerator::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    { "set-timing", 
      new MemberCall<SignalGenerator,TimeSpec>
        (&SignalGenerator::setTimeSpec), set_timing_description },

    { "check-timing", 
      new MemberCall<SignalGenerator,vector<int> >
      (&SignalGenerator::checkTiming), check_timing_description },
...
{ "amplitude",
new VarProbe<SignalGenerator, float>
(&SignalGenerator::A),
...
"Frequency of the Signal"},
    ...
    { NULL, NULL, "This is a Signal generating module"} };
...
SignalGenerator::SignalGenerator(Entity* e, const char* part, const
  ...
  t(0.5), A (0.5), f(5), mySine(0.0), myColorCounter(0),
  ...
  mySignalStreamWriteToken(getId(), NameSet(getEntity(), "SignalStream", part)),
  myDisplayColorEventWriteToken(getId(), NameSet(getEntity(), "DisplayColorEvent", part)),

  // activity initialization
  // myclock(),
  cb1(this, &SignalGenerator::doCalculation),
  do_calc(getId(), "Generates and sends a signal", &cb1, ps)
{
...
bool SignalGenerator::checkTiming(const vector<int>& i)
{
  if (i.size() == 3) {
...
bool SignalGenerator::isPrepared()
{
  ...
  CHECK_TOKEN(mySignalStreamWriteToken);
  CHECK_TOKEN(myDisplayColorEventWriteToken);
...
void SignalGenerator::doCalculation(const TimeSpec& ts)
{
  ...
  case SimulationState::HoldCurrent: {
    ...
    D_MOD("The simulation is holding the current state\n");
  // calculate sine signal value for this time instance
  // and dump it in variable called mySine
  mySine = A * sin( 2*M_PI*f*t );
    break;
    }
  ...
  case SimulationState::Advance: {
    ...
    D_MOD("The simulation is advancing to the next state\n");
    // do the simulation calculations, one step
  // update time with time step size
  t = t + ts.getDtInSeconds();
  // calculate sine signal value for this time instance
  // and dump it in variable called mySine
  mySine = A * sin( 2*M_PI*f*t );

  myColorCounter++; // increase the colour counter
  if (myColorCounter == 1000 || myColorCounter == 2000) {
  // create the EventWriter
  EventWriter<DisplayColorEvent> myDisplayColorEventWriter(myDisplayColorEventWriteToken, ts);
  // write the data to the event communication object
  if (myColorCounter == 1000) {
  myDisplayColorEventWriter.data().color = true;
  }
  else {
  myDisplayColorEventWriter.data().color = false;
  myColorCounter = 0; // start counting at 0 again
    }
  }
  ...
  StreamWriter<SignalStream> mySignalStreamWriter(mySignalStreamWriteToken, ts);
  mySignalStreamWriter.data().signal = fabs(mySine);
