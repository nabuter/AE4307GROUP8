...
"$Id: Display.cxx,v 1.1 2015/05/07 16:48:59 nabutergrebe Exp $";
...
// class/module name
const char* const Display::classname = "display";

// initial condition/trim table
const IncoTable* Display::getMyIncoTable()
{
...
//     new VarProbe<Display,double>
//       (REF_MEMBER(&Display::i_example))}
...
const ParameterTable* Display::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    { "set-timing",
      new MemberCall<Display,TimeSpec>
        (&Display::setTimeSpec), set_timing_description },

    { "check-timing",
      new MemberCall<Display,vector<int> >
      (&Display::checkTiming), check_timing_description },
    ...
    { NULL, NULL,
      "displays a variable-sized square"} };

  return parameter_table;
}

// constructor
Display::Display(Entity* e, const char* part, const
		       PrioritySpec& ps) :
 ...
 myDisplay(),mysignal(5.0),
  ...
  SSreadstreamtoken(getId(), NameSet(getEntity(), "SignalStream", part)),
  DCEreadeventtoken(getId(), NameSet(getEntity(), "DisplayColourEvent", part)),

  cb1(this, &Display::doCalculation),
  myclock(),

  do_calc(getId(), "draw display", &cb1, ps)
{
  // do the actions you need for the simulation

  // connect the triggers for simulation
  //do_calc.setTrigger( myclock );
  do_calc.setTrigger( SSreadstreamtoken );
...
bool Display::setTimeSpec(const TimeSpec& ts)
{
...
bool Display::checkTiming(const vector<int>& i)
{
  if (i.size() == 3) {
...
bool Display::isPrepared()
{
  bool res = true;

  CHECK_TOKEN(SSreadstreamtoken);
  CHECK_TOKEN(DCEreadeventtoken);
...
// start the module
void Display::startModule(const TimeSpec &time)
{
  do_calc.switchOn(time);
}

// stop the module
void Display::stopModule(const TimeSpec &time)
{
...
void Display::fillSnapshot(const TimeSpec& ts,
...
// declaration) if you specified to the SimulationModule that the size of
// state snapshots is zero
void Display::loadSnapshot(const TimeSpec& t, const Snapshot& snap)
{
...
// appropriate output
void Display::doCalculation(const TimeSpec& ts)
{
  if (DCEreadeventtoken.getNumWaitingEvents(ts))
  {
	try
	  {
		EventReader<DisplayColourEvent>  myDisplayColourReader(DCEreadeventtoken,ts);
		myDisplay.adjust(myDisplayColourReader.data().dayornight);
	  }
	catch (Exception& e)
	  {
		W_MOD(classname << ": Error while reading DisplayColourEvent!");
  ...
  case SimulationState::HoldCurrent: {
	...
	D_MOD("this is debug output(Display-Hold)");
  ...
  case SimulationState::Advance: {
	D_MOD("this is debug output(Display-ADvance)");
	try
	{
 	  StreamReader<SignalStream> mySignalReader(SSreadstreamtoken,ts);
	  mysignal = mySignalReader.data().signal;
	
	}
	catch (Exception& e)
	  {
		W_MOD(classname << ":Error while reading Streamchannel @ " << ts);
		mysignal =0.0;	
	  }
	myDisplay.setAmplitude(mysignal);
	myDisplay.redraw();
  ...
  // example
  //StreamWriter<SignalStream> mySignalWriter(signal_out, ts);
...
void Display::trimCalculation(const TimeSpec& ts, const TrimMode& mode)
{
...
static TypeCreator<Display> a(Display::getMyParameterTable());
